...
class LossLandscape:
    def __init__(self, model, trainset, criterion, weight_decay, data_decay_rate=0.1, mode="3D"):
        """
        :param model:
        ...
        labels = [self.trainset.images[i]["category_id"] for i in range(len(self.trainset))]
    ...
    @torch.no_grad()
    def draw(self, x_min=-0.1, x_max=0.1, x_interval=11, y_min=-0.1, y_max=0.1, y_interval=11):
    ...
    def test(self, temp_model: nn.Module):
        ...
        for step, (x, y) in enumerate(self.traindataloader):
            ...
            y = y.to(self.device)
            pre = temp_model(x)
            result += self.criterion(pre, y).item() * y.shape[0]
