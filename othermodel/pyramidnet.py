...
from othermodel.common import pre_conv1x1_block, pre_conv3x3_block
from othermodel.presenet import PreResActivation
import torch
...
class PyrUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bernoulli < 0:
            ...
            if self.training:
                x = ShakeDrop.apply(x, identity, self.bernoulli)
            else:
                x = x * self.bernoulli + identity
        return x
            ...
            for j, out_channels in enumerate(channels_per_stage):
                ...
                if shakedrop == False:
                    ...
                    stage.add_module("unit{}".format(j + 1), PyrUnit(
                        ...
                        stride=stride,
                        bottleneck=bottleneck,
                        bernoulli=func_number(now_number)))
                now_number += 1
                in_channels = out_channels
