...
if __name__ == "__main__":
    ...
    stage1 = (
        f"python train.py --batch_size 48 --total_epoch {args.epochs} --lr 0.1 {'--parallel' if args.parallel else ''} {'--fp16' if args.fp16 else ''} "
    ...
    stage2 = (
        f"python train.py --batch_size 48 --total_epoch {args.epochs}  --lr 0.1  {'--parallel' if args.parallel else ''} {'--fp16' if args.fp16 else ''} --kd "
        f"--img_size 224 --train_image_path {args.train_image_path} --label2id_path {args.label2id_path} --test_image_path {args.test_image_path} "
        f" --cuda_devices {args.cuda_devices}"
    )

    stage3 = (
        f"python train.py --batch_size 16 --total_epoch {args.epochs}  --lr 0.0001  {'--parallel' if args.parallel else ''} {'--fp16' if args.fp16 else ''} "
    ...
    stage4 = (
        f"python train.py --batch_size 16 --total_epoch {args.epochs} --lr 0.0001  {'--parallel' if args.parallel else ''} {'--fp16' if args.fp16 else ''} --kd "
        f"--img_size 384 --train_image_path {args.train_image_path} --label2id_path {args.label2id_path} --test_image_path {args.test_image_path} "
        f"--if_finetune --accumulate_step 4 --cuda_devices {args.cuda_devices} --warmup_epoch -1"
    )
