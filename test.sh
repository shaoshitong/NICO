...
for i in ${test_pth_path[*]}; do
  json_save_path="$label $i prediction.json"
  ...
  mv $json_save_path "$save_json_path /"
done

python ensemble.py --ensemble_path $save_json_path --save_path "$label final_prediction.json"